...
func _test_pile_tray_box_flow(controller, arm, transport, object_manager) -> void:
	...
	_expect_true(
		...
		"Arm front edge should resolve the transport tray by occupied cells."
	)
	...
	_expect_true(
		...
		"Box interaction edge should resolve receiver interface."
	)
