...
func _test_linear_model_validation() -> void:
	...
	_expect_equal(StringName(program.get_statement(0).get("vehicle_id", &"")), &"arm_vehicle", "Runtime duplicate must not mutate the source program snapshot.")
...
func _test_production_runner() -> void:
	...
	_expect_true(runner != null and compile_gate != null and manager != null and object_manager != null and selection != null, "Production scene should expose program runner dependencies.")
	...
	for _frame in range(5):
		await process_frame
	_expect_equal(runner.get_current_statement_index(), paused_statement, "PAUSED lifecycle should freeze the program counter.")
	...
	_expect_equal(selection.get_selected_vehicle(), arm, "Program commands must not mutate the player's selected vehicle.")
...
func _append_move(program: Scene01Program, vehicle_id: StringName, target: Vector2i) -> int:
	var statement_index := program.append_statement(ProgramScript.StatementType.MOVE_TO)
	program.set_statement_vehicle(statement_index, vehicle_id)
	program.set_move_target(statement_index, target)
	return statement_index


func _append_grab(program: Scene01Program, vehicle_id: StringName) -> int:
	var statement_index := program.append_statement(ProgramScript.StatementType.GRAB_DROP)
	program.set_statement_vehicle(statement_index, vehicle_id)
	return statement_index
