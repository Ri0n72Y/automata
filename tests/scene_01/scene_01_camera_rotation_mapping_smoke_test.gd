...
func _test_four_directions_preserve_click_mapping(
	...
	var test_cells := [Vector2i(1, 1), Vector2i(5, 3), Vector2i(10, 6)]
	var directions := [
...
func _test_rotation_input_actions(scene: Node, camera_rig: CAMERA_RIG_SCRIPT) -> void:
	...
	var press := _make_key_event(KEY_E, true, false)
	camera_rig._unhandled_input(press)
	_expect_equal(
		camera_rig.get_view_direction(),
		CAMERA_RIG_SCRIPT.ViewDirection.SOUTHWEST,
	...
	_expect_equal(
		camera_rig.get_view_direction(),
		CAMERA_RIG_SCRIPT.ViewDirection.SOUTHWEST,
		"Release and echo events should not rotate."
	)
	_expect_equal(emitted_directions.size(), 1, "Release and echo should not emit signals.")

	var line_edit := LineEdit.new()
	scene.add_child(line_edit)
	...
	_expect_equal(
		camera_rig.get_view_direction(),
		CAMERA_RIG_SCRIPT.ViewDirection.SOUTHWEST,
		"Focused text input should block camera rotation."
	)
	_expect_equal(emitted_directions.size(), 1, "Blocked text input should not emit signals.")
