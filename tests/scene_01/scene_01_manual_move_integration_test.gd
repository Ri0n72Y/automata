...
func _test_debug_ui_contract(scene: Node) -> void:
	...
	test.expect_true(debug_ui != null and debug_panel != null, "Debug UI should expose its collapsible structure.")
	if debug_ui == null or debug_panel == null:
		return
	for path in debug_body_paths:
		test.expect_true(scene.get_node_or_null(path) != null, "Debug UI should preserve stable path %s." % path)
	test.expect_true(bool(debug_ui.call("is_collapsed")), "Debug UI should start collapsed.")
	for path in debug_body_paths:
		var control := scene.get_node_or_null(path) as Control
		if control != null:
			test.expect_false(control.visible, "Collapsed Debug UI should hide %s." % path)
	debug_ui.call("set_collapsed", false)
	for path in debug_body_paths:
		var control := scene.get_node_or_null(path) as Control
		if control != null:
			test.expect_true(control.visible, "Expanded Debug UI should show %s." % path)
	debug_ui.call("set_collapsed", true)
