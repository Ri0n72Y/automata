...
func _run() -> void:
	...
	await process_frame
	var vehicle_manager := _scene.get_node_or_null("SceneRoot/RobotRoot/Scene01VehicleManager") as VehicleManagerScript
	var vehicle_selection := _scene.get_node_or_null("SceneRoot/GridRoot/VehicleSelectionController") as VehicleSelectionScript
	var move_controller := _scene.get_node_or_null("SceneRoot/GridRoot/VehicleMoveController") as VehicleMoveScript
	var grid_selection := _scene.get_node_or_null("SceneRoot/GridRoot/GridSelectionController") as GridSelectionScript
	_expect_true(vehicle_manager != null and vehicle_selection != null and move_controller != null and grid_selection != null, "MoveTo preparation contract test requires Scene 01 movement controllers.")
	...
	_expect_false(move_controller.request_selected_vehicle_move(Vector2i(-1, -1)), "Invalid READY MoveTo should be rejected after run preparation.")
	_expect_equal(gate.call_count, 1, "Invalid READY MoveTo should execute the run gate once.")
	_expect_equal(_scene.get_lifecycle_state(), LifecycleStateScript.State.READY, "Invalid MoveTo after preparation must keep lifecycle READY.")
	_expect_equal(arm.runtime_state.motion_state, VehicleRuntimeStateScript.MotionState.WAITING, "MoveTo preflight rejection must not mutate runtime motion state to Blocked.")
	_expect_true(arm.runtime_state.active_move_command == null, "MoveTo preflight rejection must not create an active MoveCommand.")
	_expect_equal(move_controller.get_last_rejection_reason(), &"no_path", "Invalid MoveTo should preserve the existing no_path rejection reason.")
	_expect_false(_scene.commit_gameplay_command_validation(999999), "A fabricated validation token must not bypass run preparation.")
	_expect_equal(_scene.get_lifecycle_state(), LifecycleStateScript.State.READY, "Fabricated commit token must keep lifecycle READY.")
	_expect_equal(gate.call_count, 1, "Fabricated commit token must not invoke or bypass the gate.")
	_expect_true(grid_selection.activate_live_target_mode(), "M-equivalent target mode should start after preparation and post-compile move validation.")
	_expect_equal(gate.call_count, 2, "READY M-equivalent command should execute the gate exactly once.")
	_expect_equal(_scene.get_lifecycle_state(), LifecycleStateScript.State.RUNNING, "Valid M-equivalent command should commit lifecycle RUNNING.")
	...
	_expect_equal(reset_observations.size(), 1, "Reset should publish exactly one vehicle deselection observation.")
	if reset_observations.size() == 1:
		_expect_equal(reset_observations[0], {"legacy_running": true, "lifecycle_state": LifecycleStateScript.State.RUNNING}, "Domain reset callbacks should observe legacy is_running matching the formal RUNNING state until lifecycle Reset commits.")
	_expect_false(_scene.is_running, "Legacy is_running should be false after lifecycle Reset completes.")
	_expect_equal(_scene.get_lifecycle_state(), LifecycleStateScript.State.READY, "Lifecycle should be READY after Reset completes.")
	...
	_expect_equal(_scene.get_lifecycle_state(), LifecycleStateScript.State.RUNNING, "Prepared explicit Run after Reset should enter RUNNING.")
	await _cleanup_and_finish()

func _on_vehicle_selection_changed_during_reset(_vehicle_id: StringName, has_selection: bool) -> void:
	if has_selection or _scene == null:
		return
	reset_observations.append({"legacy_running": _scene.is_running, "lifecycle_state": _scene.get_lifecycle_state()})
