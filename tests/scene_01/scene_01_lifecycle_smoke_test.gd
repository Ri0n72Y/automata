...
func _run() -> void:
	...
	_expect_equal(run_pause_button.text, "Ⅱ", "RUNNING should show pause icon.")
	...
	_expect_false(bool(grab_drop_controller.call("rotate_selected_vehicle", 1)), "Rejected run preparation should block arm rotation.")
