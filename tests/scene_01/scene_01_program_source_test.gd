...
func _init() -> void:
	...
	_test_source_diagnostics()
	_test_requirements_follow_vehicle_selection()
	if failures == 0:
...
func _test_source_round_trip() -> void:
	var source := """automata_scene01_program 1
default_vehicle arm_vehicle
select_vehicle arm_vehicle
move_to 1 3
grab_drop
select_vehicle transport_vehicle
move_to 2 3
repeat 2 4
"""
	var codec := SourceScript.new()
	var parsed := codec.parse(source)
	_expect_true(parsed["diagnostics"].is_empty(), "Valid source should parse without diagnostics.")
	...
	if second != null:
		_expect_equal(second.vehicle_id, &"arm_vehicle", "Round-trip should preserve default vehicle.")
		_expect_equal(second.nodes.size(), program.nodes.size(), "Round-trip should preserve node count.")


func _test_source_diagnostics() -> void:
	var codec := SourceScript.new()
	var parsed := codec.parse("automata_scene01_program 1\ndefault_vehicle arm_vehicle\nmove_to x 3\n")
	...
	if not diagnostics.is_empty():
		_expect_equal(int(diagnostics[0].get("line", 0)), 3, "Source diagnostic should report the failing line.")
		_expect_equal(StringName(diagnostics[0].get("code", &"")), &"move_to_syntax", "Source diagnostic should expose a stable code.")


func _test_requirements_follow_vehicle_selection() -> void:
	var program := ProgramScript.new()
	program.reset()
	program.vehicle_id = &"arm_vehicle"
	var arm_move := program.append_node(ProgramScript.NodeType.MOVE_TO)
	program.set_move_target(arm_move, Vector2i(1, 3))
	var select_transport := program.append_node(ProgramScript.NodeType.SELECT_VEHICLE)
	program.set_select_vehicle(select_transport, &"transport_vehicle")
	var transport_move := program.append_node(ProgramScript.NodeType.MOVE_TO)
	program.set_move_target(transport_move, Vector2i(2, 3))
	var select_arm := program.append_node(ProgramScript.NodeType.SELECT_VEHICLE)
	program.set_select_vehicle(select_arm, &"arm_vehicle")
