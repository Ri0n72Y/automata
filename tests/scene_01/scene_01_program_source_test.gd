...
"""
	...
	_expect_equal(int(program.get_statement(1).get("type", -1)), ProgramScript.StatementType.ROTATE, "Second statement should be Rotate.")
	_expect_equal(int(program.get_statement(1).get("turn_direction", 0)), -1, "Counterclockwise should map to one -90 degree turn.")
...
func _test_source_diagnostic_and_statement_lines() -> void:
	...
	var invalid_rotate := SourceScript.new().parse("automata_scene01_program 2\n[arm_vehicle:rotate] diagonal\n")
	_expect_equal(StringName(invalid_rotate["diagnostics"][0].get("code", &"")), &"rotate_syntax", "Invalid Rotate direction should be rejected by the source codec.")
...
func _test_requirements_follow_command_vehicle() -> void:
	...
	var arm_rotate := program.append_statement(ProgramScript.StatementType.ROTATE)
	program.set_statement_vehicle(arm_rotate, &"arm_vehicle")
	program.set_turn_direction(arm_rotate, -1)
