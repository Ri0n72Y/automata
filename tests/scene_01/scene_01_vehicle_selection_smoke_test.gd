...
func _test_selection_flow(
	...
	_expect_equal(selection.get_selected_vehicle_id(), VEHICLE_MANAGER_SCRIPT.ARM_VEHICLE_ID, "Arm should be selected.")
	_expect_true(selection.is_selection_highlight_visible(), "Selected vehicle should show a highlight.")
	_expect_true(selection.select_vehicle(transport), "Selecting another vehicle should switch selection.")
	_expect_equal(selection.get_selected_vehicle_id(), VEHICLE_MANAGER_SCRIPT.TRANSPORT_VEHICLE_ID, "Transport should replace arm selection.")
	_expect_equal(emitted.size(), 2, "Two distinct selections should emit two changes.")

	selection.cancel_selection()
	_expect_true(not selection.has_selected_vehicle(), "Cancel should clear vehicle selection.")
	_expect_true(not selection.is_selection_highlight_visible(), "Cancel should hide the highlight.")
	...
	for direction in directions:
		...
		_expect_true(
			...
			"Vehicle ray should hit the arm in direction %d." % direction
		)
		_expect_equal(
			selection.get_selected_vehicle_id(),
			VEHICLE_MANAGER_SCRIPT.ARM_VEHICLE_ID,
			"Every camera direction should resolve the same logical vehicle."
		)
	...
	_expect_equal(
		selection.get_selected_vehicle_id(),
		VEHICLE_MANAGER_SCRIPT.ARM_VEHICLE_ID,
		"Ground target clicks should preserve vehicle selection."
	)
	...
	await process_frame
	await physics_frame
	_expect_true(not selection.has_selected_vehicle(), "Grid rebuild should clear stale selection.")
	var replacement_arm = manager.get_vehicle_by_id(VEHICLE_MANAGER_SCRIPT.ARM_VEHICLE_ID)
	_expect_true(replacement_arm != null and replacement_arm != arm, "Grid rebuild should replace the Actor.")
	_expect_true(selection.select_vehicle(replacement_arm), "Replacement Actor should remain selectable.")
