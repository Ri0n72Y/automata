...
func _test_command_input_map() -> void:
	...
	test.expect_true(InputMap.has_action(VEHICLE_MOVE.STOP_TASK_ACTION), "Stop task should be an Input Map action.")
	var has_x := false
...
func _test_submission_boundaries(scene: Node, vehicle_selection, move_controller, grid_selection, manager) -> void:
	...
	move_controller.move_accepted.connect(func(vehicle_id: StringName, target: Vector2i) -> void: accepted.append({"vehicle_id": vehicle_id, "target": target}))
	move_controller.move_rejected.connect(func(vehicle_id: StringName, target: Vector2i, reason: StringName) -> void: rejected.append({"vehicle_id": vehicle_id, "target": target, "reason": reason}))
	move_controller.move_stopped.connect(func(vehicle_id: StringName) -> void: stopped.append(vehicle_id))
	...
	for case in invalid_cases:
		...
		_expect_last_rejection(rejected, VEHICLE_MANAGER.ARM_VEHICLE_ID, case["target"], VEHICLE_MOVE.REJECTION_NO_PATH, case["name"])
	...
	_test_concurrent_selection_availability(vehicle_selection, move_controller, grid_selection, arm, transport)
	await _test_manual_stop(vehicle_selection, move_controller, grid_selection, arm, transport, stopped)
	...
	_expect_last_rejection(rejected, VEHICLE_MANAGER.ARM_VEHICLE_ID, busy_target, VEHICLE_MOVE.REJECTION_BUSY, "busy vehicle")
...
func _test_concurrent_selection_availability(vehicle_selection, move_controller, grid_selection, arm, transport) -> void:
	arm.runtime_state.clear_move_command()
	...
	test.expect_true(arm.start_move(_command(arm_target, [arm_start, arm_target])), "Arm should start an independent task.")
...
func _test_manual_stop(vehicle_selection, move_controller, grid_selection, arm, transport, stopped: Array[StringName]) -> void:
	grid_selection.deactivate_live_target_mode()
	_place_vehicle(arm, Vector2i(1, 1))
	_place_vehicle(transport, Vector2i(6, 4))
	test.expect_true(arm.start_move(_command(Vector2i(5, 1), [Vector2i(1, 1), Vector2i(2, 1), Vector2i(3, 1), Vector2i(4, 1), Vector2i(5, 1)])), "Arm task should start for manual stop.")
	test.expect_true(transport.start_move(_command(Vector2i(5, 4), [Vector2i(6, 4), Vector2i(5, 4)])), "Transport task should continue independently during arm stop.")
	...
	test.expect_equal(arm.runtime_state.motion_state, RUNTIME.MotionState.MOVING, "Viewport X without a selected vehicle must not stop arm.")
	test.expect_equal(transport.runtime_state.motion_state, RUNTIME.MotionState.MOVING, "Viewport X without a selected vehicle must not stop transport.")
	test.expect_equal(stopped.size(), stopped_before, "Viewport X without a selection must not emit move_stopped.")
	...
	for case in modifier_cases:
		await _push_key(KEY_X, bool(case["shift"]), bool(case["ctrl"]), bool(case["alt"]), bool(case["meta"]))
		test.expect_equal(arm.runtime_state.motion_state, RUNTIME.MotionState.MOVING, "%s+X through Viewport must not stop the selected task." % case["name"])
		test.expect_equal(stopped.size(), stopped_before, "%s+X must not emit move_stopped." % case["name"])
	var last_reached_anchor: Vector2i = arm.runtime_state.anchor_cell
	await _push_key(KEY_X)
	test.expect_equal(arm.runtime_state.motion_state, RUNTIME.MotionState.BLOCKED, "Viewport X should stop the selected Moving task.")
	test.expect_true(arm.runtime_state.active_move_command == null, "Manual stop should clear the selected vehicle command.")
	test.expect_equal(arm.runtime_state.anchor_cell, last_reached_anchor, "Manual stop should return to the last reached anchor.")
	test.expect_equal(transport.runtime_state.motion_state, RUNTIME.MotionState.MOVING, "Manual stop must not affect another vehicle task.")
	test.expect_equal(vehicle_selection.get_selected_vehicle(), arm, "Manual stop should preserve vehicle selection.")
	test.expect_true(grid_selection.is_live_target_available(), "Stopped vehicle should immediately accept a replacement task.")
	test.expect_equal(stopped.size(), stopped_before + 1, "Manual stop should emit exactly one stop event.")
	test.expect_equal(stopped.back(), VEHICLE_MANAGER.ARM_VEHICLE_ID, "Manual stop should identify the selected vehicle.")
	await _push_key(KEY_X)
	test.expect_equal(arm.runtime_state.motion_state, RUNTIME.MotionState.BLOCKED, "Viewport X should ignore an already Blocked vehicle.")
	test.expect_equal(stopped.size(), stopped_before + 1, "Blocked duplicate X must not emit another stop event.")
	arm.reset_actor()
	await _push_key(KEY_X)
	test.expect_equal(arm.runtime_state.motion_state, RUNTIME.MotionState.WAITING, "Viewport X should ignore a Waiting vehicle.")
	test.expect_equal(stopped.size(), stopped_before + 1, "Waiting X must not emit another stop event.")
	transport.reset_actor()
...
func _test_continuous_collision_boundaries(move_controller, arm, transport) -> void:
	...
	test.expect_true(arm.start_move(_command(Vector2i(4, 3), [Vector2i(3, 3), Vector2i(4, 3)])), "Head-on arm task should start.")
	test.expect_true(transport.start_move(_command(Vector2i(5, 3), [Vector2i(6, 3), Vector2i(5, 3)])), "Head-on transport task should start.")
	...
	test.expect_true(arm.start_move(_command(Vector2i(5, 1), [Vector2i(1, 1), Vector2i(2, 1), Vector2i(3, 1), Vector2i(4, 1), Vector2i(5, 1)])), "Long arm task should start for tunneling coverage.")
	...
	test.expect_equal(arm.runtime_state.anchor_cell, Vector2i(2, 1), "Large-delta collision should retain the last safe completed anchor.")
	_place_vehicle(arm, Vector2i(1, 1))
	_place_vehicle(transport, Vector2i(4, 1))
	test.expect_true(arm.start_move(_command(Vector2i(5, 1), [Vector2i(1, 1), Vector2i(2, 1), Vector2i(3, 1), Vector2i(4, 1), Vector2i(5, 1)])), "Small-step arm task should start for frame-invariance coverage.")
	for _step in range(20):
		move_controller._physics_process(0.1)
		if arm.runtime_state.motion_state == RUNTIME.MotionState.BLOCKED:
			break
	test.expect_equal(arm.runtime_state.motion_state, RUNTIME.MotionState.BLOCKED, "Small-step traversal should detect the same static collision.")
	test.expect_equal(arm.runtime_state.anchor_cell, Vector2i(2, 1), "Collision safe anchor must be invariant across physics-frame sizes.")
	_place_vehicle(arm, Vector2i(2, 3))
	_place_vehicle(transport, Vector2i(7, 3))
	test.expect_true(arm.start_move(_command(Vector2i(3, 3), [Vector2i(2, 3), Vector2i(3, 3)])), "Early-finishing arm task should start.")
	test.expect_true(transport.start_move(_command(Vector2i(4, 3), [Vector2i(7, 3), Vector2i(6, 3), Vector2i(5, 3), Vector2i(4, 3)])), "Later transport collision task should start.")
	move_controller._physics_process(1.0)
	test.expect_equal(arm.runtime_state.motion_state, RUNTIME.MotionState.WAITING, "Vehicle completed before the collision time must remain Waiting.")
	test.expect_equal(arm.runtime_state.anchor_cell, Vector2i(3, 3), "Early-finishing vehicle should retain its completed target.")
	test.expect_equal(transport.runtime_state.motion_state, RUNTIME.MotionState.BLOCKED, "Vehicle moving into the completed vehicle should enter Blocked.")
	test.expect_equal(transport.runtime_state.anchor_cell, Vector2i(5, 3), "Later collision should retain transport's last safe completed anchor.")
	_place_vehicle(arm, Vector2i(1, 1))
	_place_vehicle(transport, Vector2i(4, 1))
	test.expect_true(arm.start_move(_command(Vector2i(2, 1), [Vector2i(1, 1), Vector2i(2, 1)])), "Edge-contact arm task should start.")
	move_controller._physics_process(0.5)
...
func _test_real_physics_frame_coordination(manager) -> void:
	...
	_place_vehicle(arm, Vector2i(3, 3))
	_place_vehicle(transport, Vector2i(6, 3))
	test.expect_true(arm.start_move(_command(Vector2i(4, 3), [Vector2i(3, 3), Vector2i(4, 3)])), "Real-frame arm collision task should start.")
	test.expect_true(transport.start_move(_command(Vector2i(5, 3), [Vector2i(6, 3), Vector2i(5, 3)])), "Real-frame transport collision task should start.")
	...
	for _frame in range(45):
		await physics_frame
		if arm.runtime_state.motion_state == RUNTIME.MotionState.BLOCKED and transport.runtime_state.motion_state == RUNTIME.MotionState.BLOCKED:
			break
...
func _push_key(keycode: int, shifted: bool = false, controlled: bool = false, alted: bool = false, metaed: bool = false) -> void:
...
func _key_event(keycode: int, pressed: bool, shifted: bool = false, controlled: bool = false, alted: bool = false, metaed: bool = false) -> InputEventKey:
...
func _select_anchor(scene: Node, grid_selection, anchor: Vector2i) -> void:
	...
	test.expect_true(grid_selection.select_from_world_position(world_position), "Anchor %s should be selectable as a ground position." % str(anchor))
	test.expect_equal(grid_selection.selected_cell, anchor, "Anchor %s should remain the selected target." % str(anchor))


func _expect_last_rejection(rejected: Array[Dictionary], expected_vehicle: StringName, expected_target: Vector2i, expected_reason: StringName, context: String) -> void:
	test.expect_false(rejected.is_empty(), "%s should emit a rejection." % context)
	if rejected.is_empty():
		return
	test.expect_equal(rejected.back(), {"vehicle_id": expected_vehicle, "target": expected_target, "reason": expected_reason}, "%s rejection payload." % context)
