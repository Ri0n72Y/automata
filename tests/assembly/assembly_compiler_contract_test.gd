...
func _test_compile_runtime_description() -> void:
	...
	var result := compiler.compile(_make_full_assembly(1))
	test.expect_true(result.success, "A valid assembly should compile.")
	...
	test.expect_equal(result.simulation_envelope.size(), 3, "Compiled envelope should aggregate occupied cells.")
	test.expect_true(result.simulation_envelope.contains_cell(Vector2i(0, 0)), "Envelope should include the drive origin cell.")
	test.expect_true(result.simulation_envelope.contains_cell(Vector2i(1, 0)), "Envelope should include the drive footprint.")
	test.expect_true(result.simulation_envelope.contains_cell(Vector2i(0, 1)), "Envelope should include the arm footprint.")
	test.expect_equal(result.metrics.cost, 35, "Assembly cost should aggregate component costs.")
	test.expect_equal(result.metrics.mass, 7.5, "Assembly mass should aggregate component mass.")
...
func _test_overlap_fails_without_partial_runtime_state() -> void:
	...
	var definition := DEFINITION_SCRIPT.new(&"overlap", 1, [first, second])
	var result := compiler.compile(definition)
	test.expect_false(result.success, "Overlapping components should fail compilation.")
	...
	test.expect_equal(result.capabilities.size(), 0, "Failed compile should not expose partial capabilities.")
	test.expect_equal(result.simulation_envelope.size(), 0, "Failed compile should not expose a partial envelope.")
	test.expect_equal(result.metrics.cost, 0, "Failed compile should not expose partial metrics.")


func _test_same_revision_reuses_cached_result() -> void:
	var compiler := COMPILER_SCRIPT.new()
	var definition := _make_full_assembly(7)
	var first := compiler.compile(definition)
	var second := compiler.compile(definition)
...
func _test_revision_change_invalidates_cache_key() -> void:
	var compiler := COMPILER_SCRIPT.new()
	var first := compiler.compile(_make_full_assembly(3))
	var second := compiler.compile(_make_full_assembly(4))
	test.expect_false(first == second, "Changing revision should produce a fresh compile result.")
	test.expect_equal(second.revision, 4, "Compile result should preserve the new revision.")
...
func _test_result_collections_are_snapshots() -> void:
	var result := COMPILER_SCRIPT.new().compile(_make_full_assembly(9))
	var occupied_cells := result.simulation_envelope.get_occupied_cells()
	occupied_cells.append(Vector2i(99, 99))
	test.expect_false(result.simulation_envelope.contains_cell(Vector2i(99, 99)), "Envelope queries should return collection snapshots.")
	var interfaces := result.get_interaction_interfaces()
