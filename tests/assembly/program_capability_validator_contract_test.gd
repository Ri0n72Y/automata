...
func _test_missing_capability_is_program_validation_failure() -> void:
	...
	var compile_result := COMPILER_SCRIPT.new().compile(DEFINITION_SCRIPT.new(&"move_only", 1, [move_only]))
	test.expect_true(compile_result.success, "Missing GrabDrop capability should not make the assembly invalid.")
	test.expect_equal(compile_result.simulation_envelope.size(), 1, "Move-only assembly should still publish collision geometry.")
...
func _test_supported_capabilities_pass_validation() -> void:
	...
	var compile_result := COMPILER_SCRIPT.new().compile(DEFINITION_SCRIPT.new(&"capable", 1, [drive]))
	var diagnostics := VALIDATOR_SCRIPT.new().validate(
		compile_result,
