...
func _ready() -> void:
	...
	_selection_highlight.set_as_top_level(true)
	set_process(false)
...
func get_selected_vehicle() -> VehicleActorScript:
	if _selected_vehicle == null or not is_instance_valid(_selected_vehicle):
		_selected_vehicle = null
		return null
	if not _is_current_vehicle(_selected_vehicle):
		_selected_vehicle = null
		return null
