...
func _ready() -> void:
	...
	_observable.configured.connect(_on_observable_configured)
	...
	_runner.execution_failed.connect(_on_program_failed)
...
func _capability_line(vehicle_id: StringName, label: String, is_arm: bool) -> String:
	...
	if _compile_gate.has_vehicle_capability(vehicle_id, &"can_move"):
		parts.append("可移动")
	parts.append("供能充足")
	if is_arm and _compile_gate.has_vehicle_capability(vehicle_id, &"can_grab"):
		parts.append("可抓取")
	if not is_arm and (
		_compile_gate.has_vehicle_capability(vehicle_id, &"can_carry")
		or _compile_gate.has_vehicle_capability(vehicle_id, &"has_tray")
	):
		parts.append("可承载")
...
func _on_observable_configured() -> void:
	if _observable.get_arm_has_item() and _runner.get_state() != RunnerScript.STATE_RUNNING:
...
func _on_standard_box_count_changed(previous_count: int, current_count: int) -> void:
	if current_count > previous_count and _runner.get_state() != RunnerScript.STATE_RUNNING:
...
func _on_program_completed() -> void:
	var basic_arm_flow := _run_seen_arm_move and _run_seen_arm_grab
	_completed_arm_program = _completed_arm_program or basic_arm_flow
	_completed_multi_vehicle_program = (
		_completed_multi_vehicle_program
		or (basic_arm_flow and _run_seen_transport_move)
	)
	_clear_run_profile()
	_evaluate_progress()
	presentation_changed.emit()
