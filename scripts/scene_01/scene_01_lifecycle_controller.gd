...
## Submit an already-constructed move from a non-input caller through the same
## lifecycle boundary used by gameplay commands. Future ProgramRunner code can
## use this entry point without bypassing run preparation, pause, capability,
## or vehicle-busy validation.
