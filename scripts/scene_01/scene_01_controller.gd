...
func initialize_grid() -> bool:
	...
	if not model.configure(grid_width, grid_height, grid_cell_size, grid_local_origin):
	...
	if not is_node_ready():
		grid_model = model
		return true
	if grid_model == null or not _has_same_runtime_grid_geometry(grid_model, model):
		_restore_runtime_grid_geometry_exports()
		push_error("Scene 01 runtime grid geometry is immutable.")
		return false
	return true
...
func world_to_grid_cell(world_position: Vector3) -> Vector2i:
	if grid_root == null or grid_model == null:
		...
		return Vector2i(-1, -1)
	return grid_model.position_to_cell(grid_root.to_local(world_position))
...
func world_to_nearest_grid_anchor(
	...
	if grid_root == null or grid_model == null:
		push_error("Scene 01 grid is not initialized.")
		return Vector2i(-1, -1)
	var anchor := grid_model.position_to_nearest_anchor(grid_root.to_local(world_position), footprint)
	return Vector2i(
...
func grid_footprint_center_to_world(anchor_cell: Vector2i, footprint: Vector2i) -> Vector3:
	if grid_root == null or grid_model == null:
		push_error("Scene 01 grid is not initialized.")
		return Vector3.ZERO
...
func get_grid_world_basis() -> Basis:
	return grid_root.global_basis if grid_root != null else Basis.IDENTITY


func get_grid_size() -> Vector2i:
	return Vector2i(grid_model.width, grid_model.height) if grid_model != null else Vector2i.ZERO


func is_grid_cell_valid(cell: Vector2i) -> bool:
	return grid_model != null and grid_model.is_cell_valid(cell)
...
func is_grid_footprint_walkable(anchor_cell: Vector2i, footprint: Vector2i) -> bool:
...
func preview_rotate_grid(direction: int) -> void:
	if grid_root == null:
		return
	grid_root.rotate_y(float(clampi(direction, -1, 1)) * PI * 0.5)
...
func _configure_grid_presentation() -> void:
	...
	_refresh_camera_for_grid()
	var active_camera: Camera3D = scene_camera_rig.get_camera() if scene_camera_rig != null else null
	if vehicle_selection_controller != null:
...
func _has_same_runtime_grid_geometry(current_model: GridModelScript, candidate_model: GridModelScript) -> bool:
...
func _refresh_camera_for_grid() -> void:
	...
	var world_center: Vector3 = grid_root.to_global((local_min + local_max) * 0.5)
