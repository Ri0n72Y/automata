...
@onready var _vehicle_label: Label = %VehicleSelectionLabel
...
func _selected_vehicle() -> VehicleActorScript:
	return _vehicle_selection.get_selected_vehicle() if _vehicle_selection != null else null

func _selected_vehicle_id() -> StringName:
	var vehicle := _selected_vehicle()
	return vehicle.get_vehicle_id() if vehicle != null else &""
...
func _on_add_move() -> void:
	...
	if vehicle_id == &"":
		return
	_append_source_line("[%s:moveTo] %d %d" % [String(vehicle_id), int(%TargetX.value), int(%TargetY.value)])

func _on_add_rotate() -> void:
	var vehicle_id := _selected_vehicle_id()
	if vehicle_id == &"":
		return
	_append_source_line("[%s:rotate] %s" % [String(vehicle_id), String(SupportScript.ROTATION_NAMES[%RotationOption.selected])])

func _on_add_grab() -> void:
	var vehicle_id := _selected_vehicle_id()
	if vehicle_id == &"":
		return
	_append_source_line("[%s:grabDrop]" % String(vehicle_id))
