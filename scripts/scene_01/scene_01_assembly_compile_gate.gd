...
func set_required_capabilities(
	...
	var normalized := _normalize_capabilities(capabilities)
	if normalized.is_empty():
		_required_capabilities.erase(vehicle_id)
	else:
		_required_capabilities[vehicle_id] = normalized
	clear_runtime_results()
...
func _requirements_for(vehicle_id: StringName) -> Array[StringName]:
	...
	if stored is Array:
		return _normalize_capabilities(stored)
	var empty: Array[StringName] = []
	return empty


func _normalize_capabilities(capabilities: Array) -> Array[StringName]:
	var result: Array[StringName] = []
	var seen: Dictionary = {}
	for capability_value in capabilities:
		var capability := StringName(capability_value)
		if capability == &"" or seen.has(capability):
			continue
