...
func _step_body(step: int) -> String:
	match step:
		...
		TutorialScript.Step.PROGRAM_RUN:
			return "在右侧 PROGRAM SOURCE 用 Arm 的 MoveTo + GrabDrop 创建并 Run 一次成功程序。可用按钮，也可直接改源码；错误会定位到物理行号。"
		TutorialScript.Step.MULTI_VEHICLE:
			return "在同一个成功程序中再加入 Transport 的 MoveTo。DSL v2 没有 SelectVehicle：每条 [vehicle_id:command] 自己写明执行车辆。"
		_:
